        ...
        provider.request(target, completion: { (result) in
            ...
            case let .success(response):
                guard let statusCode = response.response?.statusCode,
                    let url = URL(string: self.currentEndpoint) else {
                        return
                }
                self.dataDomeSdk?.handleResponse(url: url,
                                                 code: statusCode,
                                                 headers: response.response?.allHeaderFields ?? [:],
                                                 data: response.data,
                                                 fromInternal: false,
                                                 completion: { _, code, _, _ in
                                                    DispatchQueue.main.async {
                                                        self.responseLabel.text = String(format: "Response code: %d", code)
